...
my $order_search_key = "PTO-2017-FLOWERS";
...
my $orderfirstname_column = -1;
my $orderlastname_column = -1;
my $orderemail_column = -1;
my $orderphone_column = -1;

my $itemquantity_column = -1;
my $itemprice_column = -1;
...
foreach my $field (@$fields) {
	...
	if ($field eq "otime");
    $orderfirstname_column = $col
	if ($field eq "ofirstname");
    $orderlastname_column = $col
	if ($field eq "olastname");
    $orderemail_column = $col
	if ($field eq "oemail");
    $orderphone_column = $col
	if ($field eq "ophone");

    $itemquantity_column = $col
	if ($field eq "numitems");
    $itemprice_column = $col
	if ($field eq "itemamount");
...
check("orderfirstname_column");
check("orderlastname_column");
check("orderemail_column");
check("orderphone_column");

check("itemid_column");
check("itemname_column");
check("itemquantity_column");
check("itemprice_column");
...
# names.
print OUT "\"Order ID\",\"Order date\",\"Order time\",\"First name\",\"Last name\",\"Email\",\"Phone number\",\"Item\",\"Item quantity\",\"Item total amount\"," . join(",", sort(keys(%{$all_columns}))) . "\n";
...
foreach my $row (@rows) {
	...
	$$row[$orderfirstname_column] . "," .
	$$row[$orderlastname_column] . "," .
	$$row[$orderemail_column] . "," .
	$$row[$orderphone_column] . "," .
	$$row[$itemid_column] . "," .
	$$row[$itemquantity_column] . "," .
        $$row[$itemprice_column];
